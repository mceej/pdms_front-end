            ...
            <div class="dashboard-shell">
                ...
                <div class="tab-row">
                    ...
                    <div class="top-actions">
                        ...
                        <button type="button" class="secondary-btn" @click="applyDashboardFilter">Apply</button>
                    </div>
                </div>
                ...
                <section class="table-panel">
                    <div class="table-header">
                        <h2>Provinces > {{ selectedProvince }}</h2>
                    </div>
                    <table>
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            <tr v-for="province in filteredProvinces" :key="province.name">
                                <td>{{ province.name }}</td>
                                <td>{{ province.target }}</td>
                                <td>{{ province.paid }}</td>
                                <td>
                                    <div class="bar-wrap">
                                        <span :style="{ width: province.progress + '%' }"></span>
                                        <small>{{ province.progress }}%</small>
                                    </div>
                                </td>
                            </tr>
                            <tr class="total-row">
                ...
                <section class="comparison-section">
                    <div class="comparison-header">
                        ...
                        <div class="date-filter">
                            ...
                            <input v-model="dateFilterValue" type="text" placeholder="mm/dd/yyyy" />
                            <button type="button" class="small-btn" @click="applyDateFilter">Apply</button>
                        </div>
                    </div>
...
const selectedProvince = ref('Province A');
...
const dashboardData = {
    AICS: {
        ...
        paid: '-----',
        provinces: [
            { name: 'MUNI A', target: '5,890', paid: '2,500', progress: 42 },
            { name: 'MUNI B', target: '-----', paid: 'PO.00', progress: 0 },
            { name: 'MUNI C', target: '-----', paid: 'PO.00', progress: 0 },
            { name: 'MUNI D', target: '-----', paid: 'PO.00', progress: 0 },
            { name: 'MUNI E', target: '-----', paid: 'PO.00', progress: 0 },
        ],
    ...
    ECT: {
        ...
        paid: '-----',
        provinces: [
            { name: 'BRGY A', target: '4,900', paid: '2,100', progress: 38 },
            { name: 'BRGY B', target: '-----', paid: 'PO.00', progress: 0 },
            { name: 'BRGY C', target: '-----', paid: 'PO.00', progress: 0 },
            { name: 'BRGY D', target: '-----', paid: 'PO.00', progress: 0 },
            { name: 'BRGY E', target: '-----', paid: 'PO.00', progress: 0 },
        ],
...
const filteredProvinces = computed(() => dashboardData[activeTab.value].provinces);
...
const totalTableTarget = computed(() => {
    const total = filteredProvinces.value.reduce((sum, item) => sum + Number((item.target || '0').replace(/[^\d]/g, '')), 0);
    return total ? total.toLocaleString() : '-----';
});

const totalTablePaid = computed(() => {
    const total = filteredProvinces.value.reduce((sum, item) => sum + Number((item.paid || '0').replace(/[^\d]/g, '')), 0);
    return total ? total.toLocaleString() : 'PO.00';
});

const totalProgress = computed(() => {
    const average = filteredProvinces.value.reduce((sum, item) => sum + item.progress, 0) / filteredProvinces.value.length;
...
const applyDashboardFilter = () => {
    if (selectedDisasterType.value !== 'Type of Disaster') {
        alert(`Filter applied for: ${selectedDisasterType.value}`);
    }
};
