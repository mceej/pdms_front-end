  ...
  <div class="header-content">
    ...
    <div class="title-row">
      ...
      <div class="progress-box">
        ...
        <small>{{ totalPaidCount }} out of {{ totalTarget?.toLocaleString() }}</small>
      </div>
    </div>
    <div class="tab-row">
      <button
      ...
      <div v-if="activeTab === 'ECT'" class="top-actions">
        <InputText v-model="disasterName" placeholder="Type of Disaster" class="disaster-input" />
        <Button label="Apply" size="small" severity="warn" @click="applyFilters" />
      </div>
    </div>
  </div>
</header>
                    ...
                    >
                        ...
                        <template #footer>
                            <div class="total-row-footer">
                                <span>Total</span>
                                <span>{{ totalTableTarget ? totalTableTarget.toLocaleString() : '-----' }}</span>
                                <span>{{ totalTablePaid ? totalTablePaid.toLocaleString() : '-----' }}</span>
                                <div class="progress-cell">
...
const totalTarget = computed(() => apiRows.value.length || apiSummary.value.target ? apiSummary.value.target : activeData.value.target);

// FIX: sum real paid/target instead of hardcoded '-----'
const totalDisbursed = computed(() => {
    if (apiRows.value.length || apiSummary.value.amount_disbursed) return `₱${Number(apiSummary.value.amount_disbursed).toLocaleString()}`;
...
const dashboardProgress = computed(() => {
    if (apiRows.value.length || apiSummary.value.target) return apiSummary.value.progress;
...
const applyFilters = () => {
  // TODO: trigger real fetch here using activeTab, disasterName, dateFrom, dateTo
...
.total-row-footer {
    display: grid;
    grid-template-columns: minmax(180px, 1.35fr) minmax(140px, 1fr) minmax(140px, 1fr) minmax(260px, 2fr);
    align-items: center;
    gap: 14px;
    width: 100%;
    padding: 14px;
