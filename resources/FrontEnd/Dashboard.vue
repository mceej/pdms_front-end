  ...
  <div class="metric-card">
    ...
    <div class="metric-value">{{ totalPaidCount }}</div>
                ...
                <div class="table-panel">
                    ...
                    >
                        <Column field="name" header="Name" style="width: 28%" />
                        <Column field="target" header="Total Target" style="width: 24%">
                            <template #body="{ data }">
                                {{ data.target ? data.target.toLocaleString() : '-----' }}
                            </template>
                        </Column>
                        <Column field="paid" header="Paid" style="width: 11%">
                            <template #body="{ data }">
                                <div class="paid-cell">
                        ...
                        <Column header="Progress Bar" style="width: 37%">
                            <template #body="{ data }">
                ...
                <section v-else class="server-list-view">
                    ...
                    <form class="served-list-form" @submit.prevent="uploadServedList">
                        ...
                        <label class="served-field">
                            ...
                            <span class="file-dropzone">
                                ...
                                <input accept=".csv,text/csv" type="file" @change="selectServedListFile" />
                            </span>
                        </label>
    ...
    <div class="served-list-date-filter">
        ...
        <div class="date-range-inputs">
            <input
                v-model="servedListDateFrom"
                type="date"
                aria-label="Filter served lists from date"
                :max="servedListDateTo || undefined"
                @change="applyServedListDateFilter"
            />
            <span>to</span>
            <input
                v-model="servedListDateTo"
                type="date"
                aria-label="Filter served lists to date"
                :min="servedListDateFrom || undefined"
                @change="applyServedListDateFilter"
            />
            <Button label="Apply" size="small" @click="applyServedListDateFilter" />
        </div>
    </div>
</div>
...
const applyServedListDateFilter = () => {
    ...
    // Hook actual server-list filtering here once the API supports it
};
...
.municipality-search :deep(.p-inputtext) {
    width: 100%;
    font-size: 12px;
}
...
/* Center Total Target + Paid headers and their values on the same axis */
.dashboard-table :deep(.p-datatable-thead > tr > th:nth-child(2)),
.dashboard-table :deep(.p-datatable-thead > tr > th:nth-child(3)) {
    text-align: center;
}

.dashboard-table :deep(.p-datatable-thead > tr > th:nth-child(2) .p-datatable-column-header-content),
.dashboard-table :deep(.p-datatable-thead > tr > th:nth-child(3) .p-datatable-column-header-content) {
    justify-content: center;
    width: 100%;
}

.dashboard-table :deep(.p-datatable-thead > tr > th:nth-child(4)) {
    text-align: left;
    padding-left: 14px;
}

.dashboard-table :deep(.p-datatable-thead > tr > th:nth-child(4) .p-datatable-column-header-content) {
    justify-content: flex-start;
}

.dashboard-table :deep(.p-datatable-tbody > tr > td:nth-child(2)),
.dashboard-table :deep(.p-datatable-tbody > tr > td:nth-child(3)) {
    text-align: center;
}

.dashboard-table :deep(.p-datatable-tbody > tr > td:nth-child(3) .paid-cell) {
    align-items: center;
}
...
.total-row-footer {
    display: grid;
    grid-template-columns: 28% 24% 11% 37%;
    align-items: center;
    gap: 0;
    width: 100%;
    padding: 14px;
...
/* Match the footer totals to the centered Total Target and Paid columns */
.total-target,
.total-paid {
    text-align: center;
    font-weight: 900;
...
.view-tab.active {
    ...
    color: #0649b9;
    font-weight: 800;
    box-shadow: 0 1px 3px rgba(24, 67, 101, 0.18);
}
...
.served-field :deep(.p-select) {
    ...
    box-shadow: 0 1px 2px rgba(24, 67, 101, 0.06);
}
...
.file-dropzone span:not(:first-child) {
    font-size: 14px;
    font-weight: 600;
    color: #16324f;
}
...
.served-list-actions :deep(.p-button) {
    min-width: 140px;
    min-height: 40px;
    border-radius: 8px;
    font-size: 13px;
...
.served-list-table-panel {
    margin-bottom: 0;
}

.served-list-table-header {
    align-items: center;
    display: flex;
    justify-content: space-between;
    flex-wrap: wrap;
...
.served-list-table-header > span:first-child {
    color: #f5bd18;
    font-size: 0.78rem;
    font-weight: 800;
    letter-spacing: 0.06em;
...
.served-list-date-filter .date-range-inputs input {
...
.served-list-date-filter :deep(.p-button) {
    background: #f5bd18;
    border-color: #f5bd18;
    color: #0b247f;
    font-weight: 800;
...
.served-list-table :deep(.p-datatable-thead > tr > th) {
    background: #eaf1fb;
    color: #173d68;
    border-color: #c6dced;
}
