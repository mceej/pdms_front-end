...
class DatabaseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        foreach ($provinces as $index => $provinceData) {
            ...
            $secondaryMunicipality = Geography::create([
                'psgc_code' => $provinceData['code'] . '002',
                'name' => $province->name . ' Municipality 2',
                'level' => 'municipality',
                'parent_id' => $province->id,
            ]);
            $secondaryBarangay = Geography::create([
                'psgc_code' => $provinceData['code'] . '002001',
                'name' => 'Barangay Riverside',
                'level' => 'barangay',
                'parent_id' => $secondaryMunicipality->id,
            ]);
            ...
            PayoutRecord::create([
                ...
                'region_id' => $region->id,
                'province_id' => $province->id,
                'municipality_id' => $secondaryMunicipality->id,
                'barangay_id' => $secondaryBarangay->id,
                'payout_site' => 'Site 3',
                'beneficiary_reference' => "SEED-{$index}-SECONDARY",
                'target_amount' => 10000,
                'disbursed_amount' => 0,
                'is_paid' => false,
                'served_date' => now()->subDays(2),
            ]);
        }
        ...
        foreach (Geography::where('level', 'province')->where('parent_id', $region->id)->get() as $index => $ectProvince) {
            ...
            $ectSecondaryMunicipality = Geography::where('level', 'municipality')
                ->where('parent_id', $ectProvince->id)
                ->where('name', 'like', '%Municipality 2')
                ->first();
            $ectSecondaryBarangay = $ectSecondaryMunicipality
                ? Geography::where('level', 'barangay')->where('parent_id', $ectSecondaryMunicipality->id)->first()
                : null;

            if ($ectSecondaryMunicipality && $ectSecondaryBarangay) {
                PayoutRecord::create([
                    'served_list_id' => $ectList->id,
                    'program' => 'ECT',
                    'disaster_type' => 'Earthquake',
                    'region_id' => $region->id,
                    'province_id' => $ectProvince->id,
                    'municipality_id' => $ectSecondaryMunicipality->id,
                    'barangay_id' => $ectSecondaryBarangay->id,
                    'payout_site' => 'ECT Site 3',
                    'beneficiary_reference' => "ECT-SEED-{$index}-SECONDARY",
                    'target_amount' => 15000,
                    'disbursed_amount' => 0,
                    'is_paid' => false,
                    'served_date' => now()->subDays(2),
